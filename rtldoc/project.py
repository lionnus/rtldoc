...
@dataclass
class Config:
    """The settings from `rtldoc.yml`. Each one is optional."""
...
def load_config(project_root: str) -> Config:
    """Reads `rtldoc.yml`, if it is available.
...
CONFIG_TEMPLATE = """\
# RTLDoc project configuration - https://github.com/lionnus/rtldoc
...
def write_config(project_root: str, output: str = DEFAULT_OUTPUT) -> str | None:
    """Writes `rtldoc.yml`. Gives the path, or None if the file exists."""
...
def ensure_gitignored(outdir: str) -> str | None:
    ...
    block += f"\n# RTLDoc generated documentation\n{rule}\n"
...
def write_build_info(outdir: str, **fields) -> None:
    with open(os.path.join(outdir, BUILD_INFO), "w") as fh:
        json.dump({"tool": "rtldoc", **fields}, fh, indent=2)
