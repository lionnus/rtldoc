...
PACKAGE = pathlib.Path(__file__).parent.parent / "rtldoc"
...
def test_no_module_imports_the_package_itself():
    """`from rtldoc import x` inside the package makes a cycle."""
    for name in module_names():
        text = (PACKAGE / f"{name}.py").read_text()
        assert "from rtldoc" not in text, name
        assert "import rtldoc" not in text, name
...
def test_the_public_api_is_available():
    ...
    for name in rtldoc.__all__:
        assert getattr(rtldoc, name) is not None
    with pytest.raises(AttributeError):
        _ = rtldoc.not_a_real_name
